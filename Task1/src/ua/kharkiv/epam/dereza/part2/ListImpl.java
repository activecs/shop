...
import ua.kharkiv.epam.dereza.part1.NetworkEquipment;
 ...
 * @param <E>
 */
public class ListImpl<E> implements List<E>, Cloneable {
	...
	public Iterator<E> customIterator(int minPortCount, double maxWeight)
			throws CloneNotSupportedException {
		return (Iterator<E>) new CustomIterator<NetworkEquipment>(minPortCount,
				maxWeight);
	}
	...
	public class IteratorImpl<E> implements Iterator<E> {
		...
		public void remove() {
			...
			System.arraycopy(elementData, currentIndex + 1, elementData,
	...
	public class CustomIterator<E extends NetworkEquipment> implements
			Iterator<E> {
		...
		public CustomIterator(int minPortCount, double maxWeight) {
		...
		public boolean hasNext() {
			...
			while ((currentIndex + 1) < size) {
				eq = (NetworkEquipment) elementData[currentIndex + 1];
				if (eq.getWeight() <= maxWeight
						&& eq.getPortCount() >= minPortCount) {
					return true;
				} else {
					currentIndex++;
				}
