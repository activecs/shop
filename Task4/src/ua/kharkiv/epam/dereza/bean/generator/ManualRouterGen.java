...
/**
 * Full name of the class is manual router generator. It is chain of
 * responsibility pattern implementation.
 * 
 * @author Eduard_Dereza
 */
public class ManualRouterGen extends EquipmentGenerator {

	private final static String DEVICE_NAME = "Router";
	private Locale locale;

	public ManualRouterGen(Locale locale) {
		this.locale = locale;
	}
		...
		if (name.equalsIgnoreCase(DEVICE_NAME)) {
			...
			do {
				...
				try {
					...
					return new Router(model, weight, portCount, price,
							protocolType, operationSystem);
